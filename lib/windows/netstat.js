...
module.exports = (function () {
    ...
    Netstat.prototype.start = function() {
        ...
        var netstat = process.execute('netstat', ['-e'], null, null, function (err, userValue) {
            if (err) {
                $this.logger.log('error', 'windows|netstat|err=' + util.inspect(err));
                throw err;
            }
            userValue = userValue;
        ...
        netstat.stdout.on('data', function (data) {
            ...
            for (var i = 0 ; i < lines.length ; ++i) {
                ...
                $this.logger.log('debug', 'windows|netstat|stout=' + lines[i] + '|capture=' + util.inspect(capture));
        ...
        netstat.stderr.on('data', function (data) {
            $this.logger.log('debug', 'windows|netstat|stderr=' + data + '\n');
        });
    };
