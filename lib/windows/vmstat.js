...
module.exports = (function () {
    ...
    Vmstat.prototype.start = function() {
        ...
        var vmstat = process.execute('\\cygwin\\bin\\vmstat.exe', ['-n', 1, 2], null, null, function (err, userValue) {
            if (err) {
                $this.logger.log('error', 'windows|vmstat|err=' + util.inspect(err));
                throw err;
            }
            userValue = userValue;
        ...
        vmstat.stderr.on('data', function (data) {
            $this.logger.log('debug', 'windows|vmstat|stderr=' + data + '\n');
        });
        
        function processVmStatData(line) {
            var stats = {},
                capture = line.match($this.regex);
            $this.logger.log('debug', 'windows|vmstat|stout=' + line + '|capture=' + util.inspect(capture));
