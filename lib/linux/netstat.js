...
module.exports = (function () {
    ...
    Netstat.prototype.start = function() {
        ...
        var netstat = process.execute('/sbin/ifconfig', null, null, null, function (err, userValue) {
            if (err) {
                $this.logger.log('error', 'linux|netstat|err=', err);
                throw err;
            }
            userValue = userValue;
            setTimeout(function () {
        ...
        netstat.stdout.on('data', function (data) {
            ...
            try {
                for (var i = 0 ; i < lines.length ; ++i) {
                    ...
                    $this.logger.log('debug', 'linux|netstat|stout=' + lines[i] + '|interface-capture=' + util.inspect(capture));
                    if (capture !== null && capture[0] !== undefined && capture.length === 2) {
                        interfaceName = capture[1];
                        stats = {};
                    } else {
                        capture = lines[i].match($this.rxPacketsRegex);
                        $this.logger.log('debug', 'linux|netstat|stout=' + lines[i] + '|rx-capture=' + util.inspect(capture));
                        if (capture !== null && capture[0] !== undefined && capture.length === $this.rxPacketsRegexLen) {
                            ...
                            $this.logger.log('debug', 'linux|netstat|stout=' + lines[i] + '|tx-capture=' + util.inspect(capture));
                            if (capture !== null && capture[0] !== undefined && capture.length === $this.txPacketsRegexLen) {
                                ...
                                $this.logger.log('debug', 'linux|netstat|stout=' + lines[i] + '|capture=' + util.inspect(capture));
        ...
        netstat.stderr.on('data', function (data) {
            $this.logger.log('error', 'linux|netstat|stderr=' + data + '\n');
        });
    };
