...
module.exports = (function () {
    ...
    function Vmstat(samplingRate, log) {
        ...
        if (true) { // Maveric
          this.regexVmstat = /^\s*(\d+)[A-Z]?\s+(\d+)[A-Z]?\s+(\d+)[A-Z]?\s+(\d+)[A-Z]?\s+(\d+)[A-Z]?\s+(\d+)[A-Z]?\s+(\d+)[A-Z]?\s+(\d+)[A-Z]?\s+(\d+)[A-Z]?\s+(\d+)[A-Z]?\s+(\d+)[A-Z]?\s*(\d+)[A-Z]?\s*(\d+)[A-Z]?\s*(\d+)[A-Z]?\s*(\d+)[A-Z]?\s*(\d+)[A-Z]?\s*(\d+)[A-Z]?\s*(\d+)[A-Z]?\s*(\d+)[A-Z]?\s*(\d+)[A-Z]?\s*(\d+)[A-Z]?\s*(\d+)[A-Z]?\s*$/;
          this.free=1;this.active=2;this.special=3;this.inactive=4;this.wire=5;this.prgable=6;this.faults=7;this.copy=8;this.fill=9;this.reactive=10;this.pageins=18;this.pageout=19;this.regexLen=23;
    ...
    Vmstat.prototype.vm_stat = function() {
        ...
        var vmstat = process.execute('vm_stat', [$this.samplingRate], null, null, function (err, userValue) {
            if (err) {
                $this.logger.log('error', 'macosx|vm_stat|err=' + util.inspect(err));
                throw err;
            }
            userValue = userValue;
        });
        vmstat.stderr.on('data', function (data) {
            $this.logger.log('debug', 'macosx|vm_stat|stderr=' + data + '\n');
        });
        vmstat.stdout.on('data', function (data) {
            ...
            for (var i = 0 ; i < lines.length ; ++i) {
                ...
                $this.logger.log('debug', 'macosx|vm_stat|stout=' + lines[i] + '|capture=' + util.inspect(capture));
    ...
    Vmstat.prototype.iostat = function() {
        ...
        var vmstat = process.execute('/usr/sbin/iostat', [$this.samplingRate], null, null, function (err, userValue) {
            if (err) {
                $this.logger.log('error', 'macosx|iotat|err=' + util.inspect(err));
                throw err;
            }
            userValue = userValue;
        });
        vmstat.stderr.on('data', function (data) {
            $this.logger.log('debug', 'macosx|iotat|stderr=' + data + '\n');
        });
        vmstat.stdout.on('data', function (data) {
            var lines = ('' + data).split(/\r?\n/);
            ...
            for (var i = 0 ; i < lines.length ; ++i) {
                ...
                $this.logger.log('debug', 'macosx|iostat|stout=' + lines[i] + '|capture=' + util.inspect(capture));
