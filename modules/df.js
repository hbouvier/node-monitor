...
module.exports = (function () {
    ...
    Df.prototype.start = function() {
        ...
        var df = process.execute($this.command, $this.params, null, null, function (err, userValue) {
            if (err) {
                if ($this.debug) util.log('all|df|err=' + util.inspect(err));
                throw err;
            }
            userValue = userValue;
            setTimeout(function () {
        ...
        df.stdout.on('data', function (data) {
            ...
            for (var i = 0 ; i < lines.length ; ++i) {
                ...
                if ($this.debug) util.log('all|df|stout=' + lines[i] + '|capture=' + util.inspect(capture));
        ...
        df.stderr.on('data', function (data) {
            if ($this.debug) util.log('all|df|stderr=' + data + '\n');
        });
    };
