...
module.exports = (function () {
    ...
    Vmstat.prototype.start = function() {
        ...
        var vmstat = process.execute('vmstat', ['-n', $this.samplingRate], null, null, function (err, userValue) {
            if (err) {
                if ($this.debug) util.log('linux|vmstat|err=' + util.inspect(err));
                throw err;
            }
            userValue = userValue;
        });
        ...
        vmstat.stdout.on('data', function (data) {
            ...
            for (var i = 0 ; i < lines.length ; ++i) {
                ...
                if ($this.debug) util.log('linux|vmstat|stout=' + lines[i] + '|capture=' + util.inspect(capture));
        ...
        vmstat.stderr.on('data', function (data) {
            if ($this.debug) util.log('linux|vmstat|stderr=' + data + '\n');
        });
    };
