...
module.exports = (function () {
    ...
    Netstat.prototype.start = function() {
        ...
        var netstat = process.execute('/usr/sbin/netstat', ['-w', $this.samplingRate], null, null, function (err, userValue) {
            if (err) {
                if ($this.debug) util.log('macosx|netstat|err=' + util.inspect(err));
                throw err;
            }
            userValue = userValue;
            util.log('macosx|netstat|terminating normally');
        });
        ...
        netstat.stdout.on('data', function (data) {
            ...
            for (var i = 0 ; i < lines.length ; ++i) {
                ...
                if ($this.debug) util.log('macosx|netstat|stout=' + lines[i] + '|capture=' + util.inspect(capture));
        ...
        netstat.stderr.on('data', function (data) {
            if ($this.debug) util.log('macosx|netstat|stderr=' + data + '\n');
        });
    };
